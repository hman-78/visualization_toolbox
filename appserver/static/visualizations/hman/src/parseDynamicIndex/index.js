...
const _parseDynamicIndex = function(str) {
    ...
    if(parts.length > 1){                       // if parts have more than one entry, dynamic index exists
      if(isNaN(parts[1])){
            if(parts[1].includes(',')) {
    ...
    } else {                                    // if not, set to -1 which indicates that no dynamic start index exists
        seriesDataIndex.startIndex = -1;
    }

      seriesDataIndex.indices = this._parseIndex(parts[0]) // use standrd index parser to format data excluding dynamic index
  
      seriesDataIndex.fixedCount = seriesDataIndex.indices.length // fixed count of series for easier indices length access
