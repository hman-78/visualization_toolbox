const _buildDynamicOption = function (data, config) {
    ...
    for (let i = 0; i < seriesDataIndex.indices.length - 1; i++) {
    ...
    for (let i = 0; i < data.rows.length; i++) {
        for (let j = 0; j < seriesDataIndex.indices.length; j++) {
            var dataObj = {
                value: 0,
    ...
    if (seriesDataIndex.startIndex != -1) {
        ...
        for (let i = seriesDataIndex.startIndex; i < data.fields.length; i++) {
